...
void ShellCommand(char** Args, char* StdIn,
    ...
    char* ErrBuffer, size_t ErrBufferSize)
{
    ...
    // Write to StdIn
    if (StdIn != NULL) {
        write(StdInPipe[1], StdIn, strlen(StdIn)+1);
    }
    close(StdInPipe[1]);
    ...
    // Read StdOut/StdErr
    int NumBytesOut = read(StdOutPipe[0], OutBuffer, OutBufferSize);
    int NumBytesErr = read(StdErrPipe[0], ErrBuffer, ErrBufferSize);
...
int main() {
    ...
    char ErrBuffer[1024];
    char* Args[] = {"cat", NULL};
    ShellCommand(Args, "Hi",
        OutBuffer, sizeof(OutBuffer),
        ErrBuffer, sizeof(ErrBuffer));
    printf("STDOUT: %s\n", OutBuffer);
    printf("STDERR: %s\n", ErrBuffer);
    return 0;
}
